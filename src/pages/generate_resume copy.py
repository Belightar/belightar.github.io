...
def process_text_elements(soup):
    ...
    # 技能部分
    skills_items = soup.select('#experience .skill-chart-list li')
    for i, skill_item in enumerate(skills_items[:5]):
        skill_num = i + 1
        name_elem = skill_item.select_one('.skill-title')
        if name_elem:
            name_elem['data-lang'] = f'Skills.skill_{skill_num}_name'
